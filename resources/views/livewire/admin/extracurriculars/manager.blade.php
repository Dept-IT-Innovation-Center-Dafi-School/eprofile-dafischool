<div class="space-y-3">
    <div class="flex justify-between items-center">
        <h3 class="text-base font-semibold text-slate-900">Ekstrakurikuler</h3>
    ...
    <div class="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
        ...
        @forelse ($extracurriculars as $extracurricular)
            @if ($editingId === $extracurricular->id)
                <div class="p-4 bg-slate-50 space-y-3">
                    <div>
                        <label class="text-sm font-medium text-slate-700">Nama</label>
                        <input type="text" wire:model="name"
            ...
            @else
                <div class="flex items-center gap-3 px-4 py-3">
                    @if ($extracurricular->image)
                        <img src="{{ $extracurricular->image }}" alt="" class="w-10 h-10 rounded-lg object-cover shrink-0 bg-slate-100">
                    @else
                        <x-image-placeholder compact class="w-10 h-10 rounded-lg shrink-0" />
                    @endif
                    <p class="min-w-0 flex-1 text-sm font-medium text-slate-900 truncate">{{ $extracurricular->name }}</p>

                    <div class="flex items-center gap-0.5 shrink-0" role="group" aria-label="Ubah urutan {{ $extracurricular->name }}">
                        <button wire:click="moveUp({{ $extracurricular->id }})"
                                class="p-1.5 rounded-md text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition"
                                aria-label="Naikkan urutan {{ $extracurricular->name }}">
                            <x-admin.icon name="chevron-up" class="w-4 h-4" />
                        </button>
                        <button wire:click="moveDown({{ $extracurricular->id }})"
                                class="p-1.5 rounded-md text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition"
                                aria-label="Turunkan urutan {{ $extracurricular->name }}">
                            <x-admin.icon name="chevron-down" class="w-4 h-4" />
                        </button>
                    </div>

                    <div class="flex items-center gap-0.5 shrink-0 pl-1 border-l border-slate-100">
                        <button wire:click="startEdit({{ $extracurricular->id }})"
                                class="p-2 rounded-md text-slate-500 hover:text-primary-700 hover:bg-primary-50 transition"
                                aria-label="Edit {{ $extracurricular->name }}">
                            <x-admin.icon name="pencil" class="w-4 h-4" />
                        </button>
                        <button type="button"
                                x-on:click="$store.confirm.show({ title: 'Hapus {{ $extracurricular->name }}?', message: 'Data yang sudah dihapus tidak dapat dikembalikan. Ketik nama di bawah untuk konfirmasi.', confirmText: @js($extracurricular->name), onConfirm: () => $wire.delete({{ $extracurricular->id }}) })"
                                class="p-2 rounded-md text-slate-500 hover:text-red-600 hover:bg-red-50 transition"
                                aria-label="Hapus {{ $extracurricular->name }}">
