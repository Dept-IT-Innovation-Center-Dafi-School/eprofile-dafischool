<div class="space-y-3">
    <div class="flex justify-between items-center">
        <h3 class="text-base font-semibold text-slate-900">Fasilitas Jenjang</h3>
        ...
        @forelse ($facilities as $facility)
            ...
            @else
                <div class="flex items-center gap-3 px-4 py-3">
                    @if ($facility->image)
                        <img src="{{ $facility->image }}" alt="" class="w-10 h-10 rounded-lg object-cover shrink-0 bg-slate-100">
                    @else
                        <x-image-placeholder compact class="w-10 h-10 rounded-lg shrink-0" />
                    @endif
                    <p class="min-w-0 flex-1 text-sm font-medium text-slate-900 truncate">{{ $facility->name }}</p>

                    <div class="flex items-center gap-0.5 shrink-0" role="group" aria-label="Ubah urutan {{ $facility->name }}">
                        <button wire:click="moveUp({{ $facility->id }})"
                                class="p-1.5 rounded-md text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition"
                                aria-label="Naikkan urutan {{ $facility->name }}">
                            <x-admin.icon name="chevron-up" class="w-4 h-4" />
                        </button>
                        <button wire:click="moveDown({{ $facility->id }})"
                                class="p-1.5 rounded-md text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition"
                                aria-label="Turunkan urutan {{ $facility->name }}">
                            <x-admin.icon name="chevron-down" class="w-4 h-4" />
                        </button>
                    </div>

                    <div class="flex items-center gap-0.5 shrink-0 pl-1 border-l border-slate-100">
                        <button wire:click="startEdit({{ $facility->id }})"
                        ...
                        <button type="button"
                                x-on:click="$store.confirm.show({ title: 'Hapus {{ $facility->name }}?', message: 'Data yang sudah dihapus tidak dapat dikembalikan. Ketik nama di bawah untuk konfirmasi.', confirmText: @js($facility->name), onConfirm: () => $wire.delete({{ $facility->id }}) })"
